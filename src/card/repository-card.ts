...
export function createCard(data: Card): Promise<Card> {
    const isExists = cards.find((oneWord) => oneWord.word === data.word && oneWord.categoryId === data.categoryId);
    if (isExists) {
        return Promise.reject(new Error(`Category with name ${data.word} already exist`));
    }
    const newCard: Card = {
        ...data,
...
export function updateCard(data: Card): Promise<Card> {
  const cardToUpdate = cards.find((oneWord) => oneWord.word === data.word && oneWord.categoryId === data.categoryId);
